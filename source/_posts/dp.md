...
> 给定一个数字三角形，从顶部出发，每次只能向左下或右下走一步，最后到达底部，求最大和。

则状态定义 $f_{i, j}$ 表示从顶部走到 $(i, j)$ 的最大和。

状态转移方程为 $f_{i, j}=\max(f_{i-1, j-1}, f_{i-1, j})+a_{i, j}$。
...
+ 主件和附件：只买主件，买主件+某些附件（分组背包）。

+ 多叉树集合：先算子集合、再算父集合。

### 泛化物品的背包

泛化物品的背包没有固定的费用和价值，它的价值是随着分配给它的费用而定。

设价值关于费用的函数为 $h(x)$，则分配给它的费用为 $v_i$ 时，价值为 $h(v_i)$。这时，将本来固定的价值改为引用即可。

## 记忆化搜索

记忆化搜索本质上是一种按照拓扑序进行的动态规划，它的优点是可以避免重复计算。

怎么优化成记忆化搜索呢？

例如，求斐波那契数列，我们可以使用递归方法：
...
其中 $n \in \mathbb{N^*}$。这样的话，我们可以发现，当 $n$ 很大时，会有很多重复计算，比如 $fib(5)$ 和 $fib(4)$ 都会计算 $fib(3)$，这样就会造成很多重复计算。

我们可以使用记忆化搜索来优化这个问题，我们可以使用一个数组来存储已经计算过的值，这样就不会重复计算了。因为 $\forall n \in \mathbb{N^*} \implies fib(n) \neq 0$，所以我们可以使用 $0$ 来表示未使用过的值。
...
可见的是，我们增加了一个判断有没有使用过的条件，这样就可以避免重复计算了。（正规写法应该使用 `vst` 数组来记录有否使用过，但是这里为了方便，就直接用 `f` 数组来记录）

部分图上/棋盘图上的DP更适合使用记忆化搜索。

例如 [Luogu P1434 [SHOI2002] 滑雪](https://www.luogu.com.cn/problem/P1434)：
